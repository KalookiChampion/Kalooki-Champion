...
// Read a CSS pixel variable from :root (e.g. "--card-w"). Falls back to a number (px).
...
// Full-size rendered card dimensions (keep JS animations in sync with CSS).
function getCardW() { return getCssPxVar('--card-w', 100); }
function getCardH() { return getCssPxVar('--card-h', 150); }
...
function animateDrawUnified(source, seat, card, onComplete) {
  ...
  flyingCard.style.left = (sourceRect.left + sourceRect.width / 2 - (getCardW() / 2)) + 'px';
  flyingCard.style.top = (sourceRect.top + sourceRect.height / 2 - (getCardH() / 2)) + 'px';
  ...
  if (seat === 'bottom') {
    ...
    if (cards.length > 0) {
      ...
      endX = targetRect.left + targetRect.width / 2 - (getCardW() / 2);
      endY = targetRect.top;
    }
  } else {
    // For other seats, center of mini-hand area
    endX = targetRect.left + targetRect.width / 2 - (getCardW() / 2);
    endY = targetRect.top + targetRect.height / 2 - (getCardH() / 2);
  }
...
function animateDiscardUnified(seat, card, onComplete) {
  ...
  const maxX = Math.max(0, packRect.width - getCardW());
  const maxY = Math.max(0, packRect.height - getCardH());
  const endX = packRect.left + Math.random() * maxX;
  const endY = packRect.top + Math.random() * maxY;
...
function playMpDealAnimation(dealerSeat, onComplete) {
  ...
  function dealNextCard() {
    ...
    // Same CSS class as solo mode - 120x180 sizing
    const flyingCard = document.createElement('div');
    flyingCard.className = seat === 'bottom' ? 'dealing-card to-player' : 'dealing-card to-bot';
    flyingCard.style.left = (deckRect.left + deckRect.width/ 2 - (getCardW() / 2)) + 'px';
    flyingCard.style.top = (deckRect.top + deckRect.height/ 2 - (getCardH() / 2)) + 'px';
    ...
    requestAnimationFrame(() => {
      flyingCard.style.left = (targetRect.left + targetRect.width/ 2 - (getCardW() / 2)) + 'px';
      flyingCard.style.top = (targetRect.top + targetRect.height/ 2 - (getCardH() / 2)) + 'px';
...
function animateEliminationCards(cards, seat, onComplete) {
  ...
  if (seat) flyingCard.classList.add('discarded-by-' + seat);
      ...
      flyingCard.style.left = (seatRect.left + seatRect.width / 2 - (getCardW() / 2)) + 'px';
      flyingCard.style.top = (seatRect.top + seatRect.height / 2 - (getCardH() / 2)) + 'px';
...
function getRandomPackPosition(cardId) {
  if (!packCardPositions.has(cardId)) {
    const packAreaEl = document.getElementById('packArea');
    const packW = packAreaEl ? packAreaEl.clientWidth : 160;
    const packH = packAreaEl ? packAreaEl.clientHeight : 200;

    const { w: cardW, h: cardH } = getCardDims();

    // Keep the whole card inside the pack area.
    const maxX = Math.max(0, packW - cardW);
    const maxY = Math.max(0, packH - cardH);
...
function animateDiscardFromCapturedRect(card, capturedRect, seat, onComplete) {
  ...
  if (capturedRect) {
    if (seat === 'bottom') {
      ...
      startX = capturedRect.left + capturedRect.width / 2 - (getCardW() / 2);
      startY = capturedRect.top + capturedRect.height / 2 - (getCardH() / 2);
    }
  } else {
    // Fallback
...
function animateDiscardCore(seat, card, onComplete) {
  ...
  if (seat === 'bottom') {
    ...
    if (!foundCard && handEl) {
      ...
      startX = handRect.left + handRect.width / 2 - (getCardW() / 2);
      startY = handRect.top;
    }
  } else {
    ...
    if (miniEl) {
      ...
      startX = miniRect.left + miniRect.width / 2 - (getCardW() / 2);
      startY = miniRect.top + miniRect.height / 2 - (getCardH() / 2);
    }
  }
...
function animateFirstPackCard(card, onComplete) {
  ...
  // Calculate end position (near the centre of the pack area)
  const maxX = Math.max(0, packRect.width - getCardW());
  const maxY = Math.max(0, packRect.height - getCardH());
  const endX = packRect.left + (maxX / 2) + (Math.random() - 0.5) * 20;
  const endY = packRect.top + (maxY / 2) + (Math.random() - 0.5) * 20;
...
function animateDeal(cardsToDeal, index, onComplete) {
  ...
  // Create animated card element - ALL cards use standard 120x180 size
  ...
  // Position at deck - all cards use standard 120x180 size (offset 50, 75)
  dealCard.style.left = (startX - (getCardW() / 2)) + 'px';
  dealCard.style.top = (startY - (getCardH() / 2)) + 'px';
  ...
  requestAnimationFrame(() => {
    dealCard.style.left = (endX - (getCardW() / 2)) + 'px';
    dealCard.style.top = (endY - (getCardH() / 2)) + 'px';
